...
from .core.app_mgr import XHSManager

# 导入所有具体的 Action
from .actions.navigation import (
    EnsureHomeAction, EnterSearchAction, ApplyFiltersAction,
    SwipeNextVideoAction, EnterFirstVideoAction, NextXHSNoteAction
)
...
class ScoutTaskRunner:
    ...
    def __init__(self, serial=None, config_path=None):
        ...
        # 2. 应用管理层
        self.app_mgr = XHSManager(self.device)
    ...
    def run_action(self, action_class, *args, **kwargs):
        ...
        # 调用 BaseAction 封装的标准 perform 方法，将参数传给 execute
        return action_instance.perform(*args, **kwargs)

    def launch_app(self):
        """主控层封装的 APP 拉起逻辑"""
        logger.info("调度中心正在拉起小红书 APP...")
        self.app_mgr.start()
        # 等待其完全启动
